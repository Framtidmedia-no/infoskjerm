...
interface StoreItem { id: string; name: string; chainColor: string }
interface ContentItem { id: string; title: string; type: string; status: string | null; created_at: string | null }
...
const statusConfig = {
  approved: { label: "Klar", variant: "success" as const },
  ...
  scheduled: { label: "Planlagt", variant: "secondary" as const },
}
...
export function PublishWizard({ chains, tags, stores, pendingContent }: PublishWizardProps) {
  ...
  const targetCount = targetMode === "all" ? stores.length
    : targetMode === "chains" ? selectedChains.length
    : targetMode === "tags" ? selectedTags.length
    : selectedStores.length

  const selectedIds = targetMode === "all" ? []
    : targetMode === "chains" ? selectedChains
    : targetMode === "tags" ? selectedTags
    : selectedStores
    ...
    return (
      <div className="max-w-lg mx-auto text-center py-16">
        ...
        <h2 className="text-xl font-bold text-zinc-900 mb-2">
          {scheduledAt ? "Planlagt!" : "Publisert!"}
        </h2>
        <p className="text-zinc-500 mb-6">
          {scheduledAt
            ? `Innholdet publiseres automatisk ${new Date(scheduledAt).toLocaleString("nb-NO")}.`
            : "Innholdet er nå live og vil vises på skjermene innen 30 sekunder."}
        </p>
        <Button onClick={() => { setPublished(false); setStep(1); setSelectedContent([]); setScheduledAt("") }}>
          Publiser mer innhold
        </Button>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      {step === 1 && (
        <Card>
          <CardHeader><CardTitle>Hvem skal se dette innholdet?</CardTitle></CardHeader>
            ...
            {targetMode === "chains" && (
              <div>
                ...
                <div className="flex gap-3">
                  {chains.map((chain) => (
                    <button key={chain.id} onClick={() => toggle(selectedChains, setSelectedChains, chain.id)}
            ...
            {targetMode === "tags" && (
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {tags.map((tag) => (
                    <button key={tag.id} onClick={() => toggle(selectedTags, setSelectedTags, tag.id)}
                      className={`flex items-center gap-2 px-3 py-2 rounded-lg border-2 text-sm font-medium transition-all ${selectedTags.includes(tag.id) ? "border-transparent text-white" : "border-zinc-200 text-zinc-700 bg-white hover:border-zinc-300"}`}
            ...
            {targetMode === "stores" && (
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {stores.map((store) => (
                    <button key={store.id} onClick={() => toggle(selectedStores, setSelectedStores, store.id)}
      ...
      {step === 2 && (
        <Card>
          <CardHeader><CardTitle>Velg innhold å publisere</CardTitle></CardHeader>
          <CardContent className="space-y-4">
            {pendingContent.length === 0 ? (
              <p className="text-sm text-zinc-500">Ingen innhold tilgjengelig. Opprett innhold i byggeren først.</p>
            ) : (
              <div className="space-y-2">
                {pendingContent.map((item) => {
                  ...
                  const created = item.created_at ? new Date(item.created_at).toLocaleDateString("nb-NO", { day: "numeric", month: "short" }) : "—"
                  return (
                    <button key={item.id} onClick={() => toggle(selectedContent, setSelectedContent, item.id)}
                    ...
                    >
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          <span className="text-xs bg-zinc-100 text-zinc-600 px-2 py-0.5 rounded-full font-medium">{typeLabels[item.type] ?? item.type}</span>
      ...
      {step === 3 && (
        <Card>
          <CardHeader><CardTitle>Bekreft publisering</CardTitle></CardHeader>
          <CardContent className="space-y-4">
            <div className="bg-zinc-50 rounded-xl p-4 space-y-2">
              ...
              <div className="flex items-center gap-2">
                <span className="text-sm text-zinc-500 w-28">Innhold:</span>
                <span className="text-sm font-medium text-zinc-900">
                  {pendingContent.find((c) => c.id === selectedContent[0])?.title ?? "—"}
                </span>
              </div>
            </div>
            ...
            <div>
              <label className="flex items-center gap-2 text-sm font-medium text-zinc-700 mb-2">
                ...
                Planlagt publisering (valgfritt)
              </label>
              <input
                type="datetime-local"
                value={scheduledAt}
              ...
              />
              {scheduledAt && (
                <p className="text-xs text-zinc-400 mt-1">Publiseres automatisk {new Date(scheduledAt).toLocaleString("nb-NO")}</p>
              )}
            </div>
            ...
            <div className="flex justify-between pt-2">
              ...
              <Button
                ...
                disabled={publishing}
                className={scheduledAt ? "bg-blue-600 hover:bg-blue-700" : "bg-emerald-600 hover:bg-emerald-700"}
              >
                {publishing ? "Publiserer..." : scheduledAt ? (
                  <><Calendar className="w-4 h-4" /> Planlegg publisering</>
                ) : (
                  <><Globe className="w-4 h-4" /> Publiser nå</>
                )}
              </Button>
            </div>
          </CardContent>
