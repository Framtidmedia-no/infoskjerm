...
import { UserPlus, Loader2, Store as StoreIcon } from "lucide-react"
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export function InviteUserForm({
  ...
  const dirty = email.trim() !== "" || storeIds.length > 0
  ...
  return (
    <>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent
          className="max-w-md"
          // Ikke lukk på utilsiktet klikk utenfor når skjemaet er påbegynt.
          onInteractOutside={(e) => {
            if (dirty && !success) e.preventDefault()
          }}
        >
          <DialogHeader>
